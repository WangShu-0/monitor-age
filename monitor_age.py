...
class AgeMonitorApp:
    def __init__(self, root):
        ...
        self.specific_month = self.config.get("specific_month", 0)  # 0 表示所有月份，1-12 表示具体月份
    ...
    def save_config(self):
        config = {
            ...
            "specific_month": self.specific_month if self.cond_type == "每月达到年龄" else 0,
            "hour": self.hour,
            "minute": self.minute,
        }
    ...
    def create_widgets(self):
        ...
        rb_cond4 = ttk.Radiobutton(row1, text="每月达到年龄", variable=self.cond_var, value="每月达到年龄",
                                   command=self.on_cond_changed)
        rb_cond4.pack(side=tk.LEFT, padx=2)
        rb_cond5 = ttk.Radiobutton(row1, text="截止日期前达到年龄", variable=self.cond_var, value="截止日期前达到年龄",
                                   command=self.on_cond_changed)
        rb_cond5.pack(side=tk.LEFT, padx=2)

        # 第二行：指定月份（仅当“每月达到年龄”时显示）
        row_month = ttk.Frame(param_frame)
        row_month.pack(fill=tk.X, pady=2)
        self.month_frame = ttk.Frame(row_month)
        ttk.Label(self.month_frame, text="指定月份:").pack(side=tk.LEFT)
        # 使用 Spinbox 选择月份 (1-12)
        self.month_spin = ttk.Spinbox(self.month_frame, from_=1, to=12, width=4, state="normal")
        self.month_spin.pack(side=tk.LEFT, padx=5)
        self.month_spin.set("")  # 初始为空
        # 不限月份复选框
        self.unlimited_var = tk.BooleanVar(value=True)  # 默认不限
        self.unlimited_cb = ttk.Checkbutton(self.month_frame, text="不限月份", variable=self.unlimited_var,
                                            command=self.on_unlimited_changed)
        self.unlimited_cb.pack(side=tk.LEFT, padx=5)
        ttk.Label(self.month_frame, text="(勾选\"不限月份\"则提醒所有月份出生的人员)").pack(side=tk.LEFT, padx=5)
        self.month_frame.pack_forget()

        # 第三行：截止日期输入（仅当条件为截止日期时显示）
        row_deadline = ttk.Frame(param_frame)
        row_deadline.pack(fill=tk.X, pady=2)
        self.deadline_frame = ttk.Frame(row_deadline)
        ...
        self.month_var_deadline = tk.IntVar(value=self.deadline_date.month)
        self.month_spin_deadline = ttk.Spinbox(self.deadline_frame, from_=1, to=12,
                                               textvariable=self.month_var_deadline, width=4)
        self.month_spin_deadline.pack(side=tk.LEFT, padx=2)
        self.month_spin_deadline.bind('<FocusOut>', lambda e: self.on_date_changed())
        self.month_spin_deadline.bind('<Return>', lambda e: self.on_date_changed())
        ...
        # 第四行：每日定时
        row_time = ttk.Frame(param_frame)
        row_time.pack(fill=tk.X, pady=2)
        ttk.Label(row_time, text="每日定时:").pack(side=tk.LEFT)
        self.hour_var = tk.IntVar(value=self.hour)
        hour_spin = ttk.Spinbox(row_time, from_=0, to=23, width=3, textvariable=self.hour_var)
        hour_spin.pack(side=tk.LEFT)
        ttk.Label(row_time, text=":").pack(side=tk.LEFT)
        self.minute_var = tk.IntVar(value=self.minute)
        minute_spin = ttk.Spinbox(row_time, from_=0, to=59, width=3, textvariable=self.minute_var)
        ...
        self.update_extra_widgets()
        self.on_sheet_mode_changed()
        self.refresh_sheet_list()

    def on_unlimited_changed(self):
        """复选框改变时，启用/禁用月份 Spinbox"""
        if self.unlimited_var.get():
            self.month_spin.config(state="disabled")
            self.month_spin.set("")
        else:
            self.month_spin.config(state="normal")
            if not self.month_spin.get():
                self.month_spin.set(1)

    def update_extra_widgets(self):
        """根据监测条件显示/隐藏指定月份和截止日期控件"""
        cond = self.cond_var.get()
        if cond == "每月达到年龄":
            self.month_frame.pack(fill=tk.X, pady=2)
            # 根据当前配置初始化复选框和spinbox状态
            if self.specific_month == 0:
                ...
                self.month_spin.config(state="normal")
                self.month_spin.set(self.specific_month)
        else:
            self.month_frame.pack_forget()
        ...
        if cond == "截止日期前达到年龄":
            ...
            for child in self.deadline_frame.winfo_children():
                if isinstance(child, ttk.Spinbox) or isinstance(child, ttk.Button):
    ...
    def update_day_range(self):
        ...
        month = self.month_var_deadline.get()
    ...
    def on_date_changed(self, event=None):
        ...
        try:
            ...
            if new_date != self.deadline_date:
                ...
                self.log(f"截止日期未变化: {self.deadline_str}")
    ...
    def load_settings_to_ui(self):
        ...
        # 指定月份（将在 update_extra_widgets 中根据 specific_month 设置）
        self.specific_month = self.config.get("specific_month", 0)
        # 截止日期
        self.year_var.set(self.deadline_date.year)
        self.month_var_deadline.set(self.deadline_date.month)
    ...
    def get_current_settings(self):
        ...
        # 指定月份
        if self.cond_type == "每月达到年龄":
            if self.unlimited_var.get():
                self.specific_month = 0
            else:
                try:
                    month = int(self.month_spin.get())
                    if 1 <= month <= 12:
                        self.specific_month = month
                    else:
                        self.specific_month = 0
                except:
                    self.specific_month = 0
        ...
        if self.cond_type == "截止日期前达到年龄":
            try:
                self.deadline_date = datetime(self.year_var.get(), self.month_var_deadline.get(), self.day_var.get()).date()
                self.deadline_str = self.deadline_date.strftime("%Y-%m-%d")
            except:
                pass
        self.save_config()
    ...
    def is_condition_met(self, target_date, birth_date, today):
        cond = self.cond_type
        if cond == "本月达到年龄":
            ...
            age_now = self.compute_age(birth_date)
            if age_now < self.target_age:
                return False
            if self.specific_month:
                return birth_date.month == self.specific_month
            else:
                return True
        elif cond == "截止日期前达到年龄":
    ...
    def check_new_targets(self):
        ...
        for sheet in sheets_to_read:
            ...
            if name_col is None or birth_col is None:
                self.log(f"工作表 {sheet} 中未找到姓名或出生日期列，表头列: {list(df.columns)}")
                continue
            ...
            for idx, row in df.iterrows():
                ...
                if self.is_condition_met(target_date, birth, today):
        ...
        if all_results:
            ...
            self.log(f"共 {len(all_results)} 人，其中即将达标: {future_count}")
